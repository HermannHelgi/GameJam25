...
func play_walking_sound() -> void:
	audioEmitter.stream = walkingSounds[randi_range(0,walkingSounds.size())]
	audioEmitter.play()
	
func play_destroying_sound() -> void:
	audioEmitter.stream = destroyingSounds[randi_range(0, destroyingSounds.size())]
	audioEmitter.play()
	
func play_laughing_sound() -> void:
	audioEmitter.stream = laughingSounds[randi_range(0, laughingSounds.size())]
	audioEmitter.play()
	
func play_loosingItem_sound() -> void:
	audioEmitter.stream = loosingItemSound[randi_range(0, loosingItemSound.size())]
	audioEmitter.play()
